...
export default function Projects() {
    ...
    return (
        <div className='w-11/12 m-auto px-10 min-h-screen'>
            ...
            {porjects.map((p) => {
                return (<div className="px-8 pt-10 flex">
                    <Link to={p.projectURL} target='_blank'>
                        <img src={p.projectImage} className='w-11/12 rounded-md hover:scale-105 transition' alt="ProjectImg" />
                    </Link>
                    <div>
                        <h3 className='c-400 text-5xl font-medium'>{p.projectName}</h3>
                        <p className='c-200 text-2xl font-medium py-5'>{p.porjectDiscreption}</p>
                        {p.LiveDemoURL ?
                            <Link to={p.LiveDemoURL} className='c-200 font-semibold' target='_blank'>Live Demo</Link> : ""}
                        {p.GithubURL ?
                            <Link to={p.GithubURL} className='c-200 ml-4 font-semibold' target='_blank'>Github</Link> : ""}
                    </div>
                </div>)
            })}
        </div>
